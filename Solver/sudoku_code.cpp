...
#define valCount(x) ( popcount(x) )
#define valSmallest(x) ( _tzcnt_u64(x) + 1 ) // Using _tzcnt_u64 for trailing zeroes count

// Function to count the number of set bits (1-bits) in a 64-bit integer
...
int main(int argc, char *argv[]){
   if (argc<1){
      printf("Usage: ./sudoku <thread_count>\n");
      exit(0);
   }
   int **originalGrid = readInput();
   thread_count = 4;
   if (thread_count<=0){
